...
class Server:
    def __init__(self, host, port):
        ...
        self.client_list = []               # list of active clients
        self.host = host
        self.port = port
    ...
    def accept_connection(self):            # accept new connection
        ...
        client = ConnectedClient(self.client_counter, connection, addr, self.init_time, self.packet_queue, self.close_connection)
    ...
    def close_connection(self, num):        # remove client which number is 'num'
        ...
        if len(self.client_list) == 0:      # if no client is connected -> stop writing file
            self.packet_queue.put((-1, b'dummy', b'dummy'))
...
class ConnectedClient:
    ...
    def message_exhange_thread(self):
        ...
        try:
            ...
            if Values.HANDSHAKE_KEY == received_handshake_key:
    ...
    def get_packet(self):
        ...
        while True:
            pkt = self.connection.recv(Values.BUFFER_SIZE)
            if len(pkt) == 0:
